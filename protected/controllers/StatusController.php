...
class StatusController extends Controller
{
	...
	$model=new Status;
        ...
        $model->designation=$user->publisher;
        $model->status=$user->parent;
...
//        $model->image = $fileName;
		...
		if(isset($_POST['Status']))
		{
            ...
            if($model->save()) {
                ...
                $this->redirect(array("status/index"));
            }

				$this->redirect(array('view','id'=>$model->serial));
		}
...
//        print_r($designation);
        if ($designation == "Admin"){
            $dataProvider=new CActiveDataProvider('Status', array(
                'criteria' => array('condition' => "privacy='public' or share='$user_id'")));
        }elseif($designation == "Agent"){
            $dataProvider=new CActiveDataProvider('Status', array(
                'criteria' => array('condition' => "user_id='$user_id' or status='$show' && privacy='public' or share='$user_id'")));
        }else{
            $dataProvider=new CActiveDataProvider('Status', array(
//               'criteria' => array('condition' => "designation='$designation' && privacy='public' or share='$user_id'")));
            'criteria' => array('condition' => "user_id='$user_id' or share='$user_id'")));
